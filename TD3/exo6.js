...
// --- Smoke explosion setup (from TD3 - Exercice 5) ---
const SMOKE_IMAGE_COUNT = 91;
const SMOKE_IMAGE_PATH = './assets/';
const SMOKE_FRAME_RATE = 30; // fps
const SMOKE_BASE_OPACITY = 0.22;
const SMOKE_SPRITE_SIZE = 1.0;

const smokeMaps = [];
const smokeLoader = new THREE.TextureLoader();
for (let i = 0; i < SMOKE_IMAGE_COUNT; i++) {
    const file = SMOKE_IMAGE_PATH + String(i).padStart(4, '0') + '.png';
    const tex = smokeLoader.load(file);
...
const activeSmokeSprites = [];
const frameInterval = 1 / SMOKE_FRAME_RATE;
let accumulator = 0;

function spawnSmokeExplosion(origin) {
    const numSprites = 50; // localized burst
    for (let i = 0; i < numSprites; i++) {
        const startIndex = Math.floor(Math.random() * SMOKE_IMAGE_COUNT);
        const mat = new THREE.SpriteMaterial({
            map: smokeMaps[startIndex],
            color: 0xffffff,
        ...
        const spread = 0.6;
        s.scale.set(SMOKE_SPRITE_SIZE, SMOKE_SPRITE_SIZE, 1);
        s.position.set(
        ...
        s.userData = {
            frame: startIndex,
            frameSpeed: 1 + Math.random() * 1.5,
            life: 0,
            lifeMax: 0.7 + Math.random() * 0.6,
...
document.addEventListener('keydown', (event) => {
    if (event.key === 'f' && bullet && actions['ThumbsUp'] && !isFiring) {
        ...
        setTimeout(() => {
            bullet.fire();
            // Spawn localized smoke explosion at muzzle
            if (bullet && bullet.bullet) {
                spawnSmokeExplosion(bullet.bullet.position.clone());
            }
...
gsap.ticker.add(() => {
    ...
    // Update smoke sprites at fixed frame rate
    accumulator += delta;
    while (accumulator >= frameInterval) {
        for (let i = activeSmokeSprites.length - 1; i >= 0; i--) {
            const sp = activeSmokeSprites[i];
            const ud = sp.userData;
            // advance animated frame
            ud.frame = (ud.frame + ud.frameSpeed) % SMOKE_IMAGE_COUNT;
            const nextIndex = Math.floor(ud.frame) % SMOKE_IMAGE_COUNT;
            const nextMap = smokeMaps[nextIndex];
            if (sp.material.map !== nextMap) {
                sp.material.map = nextMap;
                sp.material.mapNeedsUpdate = true;
            }
            // motion and growth
            sp.position.add(ud.velocity.clone().multiplyScalar(frameInterval));
            ud.velocity.multiplyScalar(0.92);
            const growth = 1 + 0.85 * frameInterval;
            const newScale = Math.min(ud.startScale * 2.0, sp.scale.x * growth);
            sp.scale.set(newScale, newScale, 1);
            // fade
            ud.life += frameInterval;
            const alpha = Math.max(0, 1 - ud.life / ud.lifeMax);
            sp.material.opacity = SMOKE_BASE_OPACITY * alpha;
            // cleanup
            if (ud.life >= ud.lifeMax) {
                scene.remove(sp);
                activeSmokeSprites.splice(i, 1);
            }
        }
